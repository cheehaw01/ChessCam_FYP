...
class Board:
    """
    This class will holds all the Square instances and updates changes to board after moves
    """
    ...
    def determineChanges(self, previous, current):
        ...
        squareOne.draw(copy, (255, 0, 0), 2)
        squareTwo.draw(copy, (255, 0, 0), 2)
        cv2.namedWindow('previous', cv2.WINDOW_NORMAL)
        cv2.resizeWindow("previous", 400, 400)
        cv2.imshow("previous", previous)
        cv2.namedWindow('identified', cv2.WINDOW_NORMAL)
        cv2.resizeWindow("identified", 400, 400)
        cv2.imshow("identified", copy)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        ...
        if distCurr1 < distCurr2:
            ...
            if squareTwo.state.lower() == 'p':
                if squareOne.position[1:2] == '2' and squareTwo.position[1:2] == '1':
                    self.promo = True
                    DataModule.setPromotion(True)
                    promoPiece = DataModule.readPromotionNewPiece()
                    while (promoPiece == ""):
                        promoPiece = DataModule.readPromotionNewPiece()

                    # promoChoice = input ("\nBlack Pawn Promotion! What do you replace? Queen(q), Rook(r), Bishop(b) or Knight(n) ")
                    # squareTwo.state = promoChoice
                if squareOne.position[1:2] == '7' and squareTwo.position[1:2] == '8':
                    ...
                    # promoChoice = input ("\nWhite Pawn Promotion! What do you replace? Queen(q), Rook(r), Bishop(b) or Knight(n) ")
                    # squareTwo.state = promoChoice
